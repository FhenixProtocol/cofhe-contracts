...
### Added
- **Block-explorer verification tooling** — `hardhat-verify` (Etherscan V2, single `ETHERSCAN_API_KEY`) is now wired into both internal Hardhat projects. host-chain adds `pnpm task:verifyDeployment` (verifies the TaskManager, ACL and PlaintextsStorage implementations plus proxy links; discovers ACL/PlaintextsStorage addresses on-chain and verifies the CreateX-deployed TaskManager proxy source directly), and registry-chain adds `pnpm task:verify` for the CommitmentRegistry. See `contracts/internal/VERIFICATION.md`. An AI security audit of the deployed contracts is recorded under `audits/`.
