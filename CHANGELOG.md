...
### Fixed
- `verifyInput` now respects the `isEnabled` kill-switch (`onlyIfEnabled`). Previously it granted transient ACL access even while the TaskManager was disabled, because unlike `createTask` it does not route through `sendEventCreated` (which carries `onlyIfEnabled`) and, unlike the recently fixed `createRandomTask`, it did not carry its own guard. It now reverts with `CofheIsUnavailable` when disabled, so all three access-list-gated intake paths (`createTask`, `createRandomTask`, `verifyInput`) behave consistently during a halt. Added kill-switch regression tests covering `verifyInput` and `createTask` (previously only `createRandomTask` was tested).
