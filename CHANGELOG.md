...
### Changed
- **BREAKING — role-based access control.** `TaskManager`, `ACL`, `PlaintextsStorage` and `CommitmentRegistry` move from `Ownable` to `AccessControlDefaultAdminRules`. `initialize` signatures changed and deploys must grant the operational roles explicitly; migrate live proxies with `initializeV2`.
- **Aggregator allowlist removed** — `addAggregator`, `removeAggregator`, `handleDecryptResult` and `handleError` are gone; decrypt results publish only through the signature-checked path. The mapping stays as reserved storage, renamed `_aggregators`.

### Fixed
- `initializeV2` seeds `verifierSigner` and `decryptResultSigner` to `address(1)` when it reads zero, so a bootstrap migration can't land with verification silently disabled.
- `task:upgradeTM` no longer aborts on the `DeterministicTM` → `TaskManager` migration, which is knowingly layout-incompatible.
- Deploy scripts reject blank admin delays — `TM_ADMIN_DELAY=""` previously became `0`.
...
- **ACP (Permit V3)** — scoped, revocable permissions: new `ACP` struct, `PermissionedUpgradeable` base, and `ACL.isAllowedWithPermission`. Requires an ACL upgrade.
- **`ACPShareRegistry`** — new UUPS contract for sharing an ACP with a recipient on-chain. Needs a deploy, plus its address set on the ACL.
- **`div`, `rem`, `mul` and `square` bindings for `euint64`/`euint128`** — `a.div(b)` now compiles, not just `FHE.div(a, b)`.
...
- **`FHE.sol` pragma floor is now `>=0.8.25`** to match the `ICofhe.sol` it imports; nothing below 0.8.25 could compile before.

## v0.1.5 - 2026-08-16 - deployed to testnet-v2 2026-08-16
