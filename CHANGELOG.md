...
- **ACP (Permit V3) — scoped, revocable permissions.** Adds the `ACP` struct and `PermissionedUpgradeable` base (global / per-contract / per-handle scopes, EIP-712 domain v2, revocation via `ACPTimestampRevoker`) and `ACL.isAllowedWithPermission`, whose scopes narrow an issuer's existing grants and never widen them. Requires an ACL upgrade.
- **`ACPShareRegistry`** — UUPS registry for handing an ACP to a recipient on-chain (`share`, `removeShare`, `sharesFor`, `getShare`, `isShareValid`), replacing the off-chain JSON copy-paste flow; `sharesFor` returns only unexpired, unrevoked shares. Needs a fresh deploy and its address set on the ACL.
- **`div`, `rem`, `mul` and `square` method bindings for `euint64` and `euint128`** — `library FHE` already exposed all six, so `FHE.div(a, b)` compiled while `a.div(b)` did not. Forwarders only; no change to the underlying operations.
...
- **`FHE.sol` pragma floor raised to `>=0.8.25`** (from `>=0.8.19`) to match the `ICofhe.sol` it imports. The old range was unbuildable below 0.8.25 — solc 0.8.20 failed with a ParserError — so this documents the real requirement rather than tightening it.
