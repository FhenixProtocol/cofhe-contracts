...
/**
 * @notice Receiver that follows the call-edge rule: the handle arrives as a parameter, so the
 *         sharer must be the caller. Uses receiveEuint64Param().
 */
contract SharedToken is IPullTarget {
    ...
    /// @dev Test hook: makes pull() revert for one specific caller, so a sharer can be driven down
    ///      a try/catch path and leave its share unconsumed.
...
/**
 * @notice Same receiver, spelled the way the docs forbid: names a trusted sharer instead of the
 *         party that delivered the handle. Exists so the presenter-substitution test can prove the
 *         attack is otherwise live — without it, asserting "SharedToken reverts" would pass
 *         vacuously whenever the setup is subtly wrong.
 */
...
/**
 * @notice Sharer side. Holds per-user encrypted balances and exercises each way a share can be
 *         produced.
 */
contract SharedVault {
    ...
    /// @notice Full round trip: share out, receive the result back off the same call.
    ...
    /// @notice Shares, then swallows a failing call — leaves the slot live for the rest of the
    ///         transaction.
    ...
    /// @notice Shares without calling anyone. Used for cross-transaction expiry.
    function shareOnly(IPullTarget token, address user) external {
        FHE.shareEuint64(balances[user], address(token));
    }

    /// @notice One share, two receives — the second must fail.
    ...
    /// @notice Directs the share at `other`, then hands it to `token` anyway.
    ...
    /// @notice Attempts to share a handle this contract was never allowed on.
    ...
    /// @notice Hands over a handle nobody shared.
...
/**
 * @notice Presenter substitution. Not a reentrancy: the attacker orchestrates the transaction,
 *         calling the vault and then the token in sequence. The share written during the first call
 *         is still live during the second.
 */
