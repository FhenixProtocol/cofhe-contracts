 ...
 * `revokerData` as its creation timestamp (populated by the SDK at permission
 * creation, backdated 60s for clock skew). Every SDK-created permission points
 * here by default: no upfront contract call is needed — a permission is
 * revocable from birth.
 *
 * `disabled(issuer, id)` truth table:
 *
 *   | condition                   | result | meaning                          |
 *   |-----------------------------|--------|----------------------------------|
 *   | id > block.timestamp        | true   | future-dated permissions never   |
 *   |                             |        | validate (closes the revoke-all  |
 *   |                             |        | dodge: a permission dated ahead  |
 *   |                             |        | of a mass revocation would       |
 *   |                             |        | otherwise survive it)            |
 *   | id <= revokeAllAt[issuer]   | true   | mass revocation (inclusive)      |
 *   | revokedSingle[issuer][id]   | true   | targeted revocation              |
 *   | otherwise                   | false  | permission valid                 |
 *
 * Accepted trade-offs (by design, see ACP spec discussion):
 *  - Two permissions minted by the same issuer in the same second share an id;
 *    revoking one revokes both. Over-revocation is the fail-safe direction.
 *  - Revocation is permanent — there is no un-revoke.
 *
 * @dev TODO(review): production wants this upgradeable (OZ AccessControl for the
...
contract ACPTimestampRevoker is IPermissionCustomIdValidator {
    /// @notice issuer => threshold; permissions with id (creation ts) at or before this are revoked
    ...
    /// @notice Revoke a single permission by its id (creation timestamp).
    ///         Only affects permissions issued by the caller.
    function revokeSingle(uint256 id) external {
        revokedSingle[msg.sender][id] = true;
        emit RevokedSingle(msg.sender, id);
    }

    /// @notice Revoke every permission the caller created up to now.
    ///         O(1): a single threshold write, regardless of permission count.
